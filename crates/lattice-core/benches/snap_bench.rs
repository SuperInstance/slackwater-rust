...
use lattice_core::{EisensteinPoint, snap_all, snap_position};
...
fn bench_neighbors(c: &mut Criterion) {
    ...
    c.bench_function("neighbors", |b| b.iter(|| black_box(p.neighbors())));
}

fn bench_within(c: &mut Criterion) {
    let p = EisensteinPoint::origin();
    c.bench_function("within_radius_5", |b| b.iter(|| black_box(p.within(5))));
}
